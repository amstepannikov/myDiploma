...
* Создаём приложение my_blog
   * окна пользователя
     ...
     * ввод нового пароля - 10%
       * проверка пароля - 10%
         ...
         * проверка наличия типов символов (большая, маленькая, цифры, спец-символы) - 100%
         * повторный ввод пароля, должен равен первому - 100%
         * кнопка показать/скрыть пароль - 20%
         * генератор пароля - 20%
         * отображение сложности пароля - 20%
   ...
   * админка - 20%
     ...
     * проверки (только для роли super)
       * проверка email на утечку через API [Have I Been Pwned API v3](https://haveibeenpwned.com/API/v3#BreachesForAccount) - 1%
       * проверка на утечки паролей - 1%
...
* регистрация через GitHub - 5%
* проверки (только для роли super)
  * проверка email на утечку через API - 1%
  * проверка на утечки паролей - 1%
остальное мелочи  
